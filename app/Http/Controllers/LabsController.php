...
class LabsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $cacheKey = "ab_testing_v7_{$startDate->format('Y-m-d')}_{$endDate->format('Y-m-d')}_{$sourceKey}";
    ...
    public function clearCache(Request $request)
    {
        ...
        $sourceKey = $sourceFilter ?? 'all';
        Cache::forget("ab_testing_v7_{$startDate->format('Y-m-d')}_{$endDate->format('Y-m-d')}_{$sourceKey}");
