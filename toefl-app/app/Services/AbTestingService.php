...
class AbTestingService
{
    ...
    {
        $counts = $this->batchEventCounts($startDate, $endDate, $sourceFilter,
    ...
    public function getCtaPerformance(Carbon $startDate, Carbon $endDate, ?string $sourceFilter = null): array
    {
        ...
        $paymentSessions = DB::table('user_analytics')
            ->where('event_type', 'payment')
            ->whereBetween('created_at', [$startDate, $endDate])
            ->distinct()
            ->pluck('session_id');

        return $ctaClicks->groupBy('landing_source')->map(function ($sourceClicks, $landingSource) use ($paymentSessions) {
            $locations = $sourceClicks->groupBy('cta_location')->map(function ($locationClicks, $location) use ($paymentSessions) {
                ...
                return [
                    ...
                    'conversions'     => $conversions,
                    'conversion_rate' => round($this->safeDiv($conversions, $uniqueSessions->count()) * 100, 2),
                ];
    ...
    private function batchBouncedCounts(Carbon $startDate, Carbon $endDate, ?string $sourceFilter): array
    {
        ...
        return $rows->pluck('bounced', 'landing_source')->all();
    }
    ...
    private function batchRevenue(Carbon $startDate, Carbon $endDate, ?string $sourceFilter): array
    {
        ...
        return $rows->pluck('revenue', 'landing_source')->all();
    }
